...
export default function CustomerBiltyListScreen() {
    ...
    useEffect(() => {
        const load = async () => {
            try {
                ...
                if (!partyId) {
                    setError('No party is linked to this user. Please contact support.');
                    setBilties([]);
                    return;
                }

                const data = await fetchBiltiesForConsignee(partyId);
                setBilties(data);
            } catch (e: any) {
                console.error('[CustomerBiltyList] Error loading bilties', e);
        ...
        const filteredBilties = bilties.filter((b) => {
            ...
            if (recentFilter !== 'all') {
                ...
                const days = recentFilter === '7d' ? 7 : recentFilter === '30d' ? 30 : 0;
                if (days > 0) {
    ...
    return (
        <View style={styles.container}>
            <Text style={styles.title}>My Bilties</Text>
            <TextInput
